...
export const metadata: Metadata = {
  title: "Archivo — Elim LLDM",
  description: "Pláticas grabadas de la fe LLDM. Accede a todo el archivo de enseñanzas, testimonios, profecías y estudios bíblicos.",
};
...
export default async function ArchivoPage({
  ...
  let query = supabase
    .from("archive")
    .select("id, title, description, thumbnail_url, duration_seconds, view_count, published_at, tags, categories(id, name, slug)")
      ...
      >
        <div className="max-w-5xl mx-auto">
          ...
          <p style={{ color: "var(--color-text-muted)" }}>
            Pláticas grabadas de la fe LLDM — siempre disponibles
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {(categories ?? []).length > 0 && (
          <div className="flex gap-2 flex-wrap mb-6">
            ...
            {(categories as Array<{ id: string; name: string; slug: string }>).map((c) => (
              <Link
                key={c.id}
                href={`/archivo?categoria=${c.slug}`}
                className="px-3 py-1.5 rounded-full text-xs font-medium transition-all"
                style={{
                  background: activeCategory && (activeCategory as { id: string }).id === c.id
                    ? "var(--color-primary)"
                    : "var(--color-surface)",
                  color: activeCategory && (activeCategory as { id: string }).id === c.id
                    ? "#000"
        ...
        {tag && (
          <div className="flex items-center gap-2 mb-6">
            ...
            <Link href="/archivo" className="text-xs" style={{ color: "var(--color-text-muted)" }}>
              × quitar
            </Link>
          </div>
        )}
        ...
        {(q || categoria || tag) && (
          <p className="text-sm mb-4" style={{ color: "var(--color-text-muted)" }}>
            ...
            {q && <> para "<span style={{ color: "var(--color-text)" }}>{q}</span>"</>}
          </p>
        )}
        ...
        {(!items || items.length === 0) ? (
          ...
          >
            ...
            <p style={{ color: "var(--color-text-muted)" }}>
              {q || categoria || tag
                ? "No se encontraron pláticas con esos filtros"
                : "El archivo estará disponible pronto"}
            </p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(items as unknown as Array<{
              ...
              <ArchiveCard key={item.id} item={item} activeTag={tag} />
            ))}
          </div>
        )}
      </div>
