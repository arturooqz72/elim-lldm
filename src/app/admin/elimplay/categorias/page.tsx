...
export const metadata = { title: "Categorías de audio — Admin" };
...
async function createCategory(formData: FormData) {
  ...
  await service.from("audio_categories").insert({ name, slug, description, icon, order_index });
  revalidatePath("/admin/elimplay/categorias");
}
...
async function deleteCategory(formData: FormData) {
  ...
  if (!id) return;
  const service = await createServiceClient();
  await service.from("audio_categories").delete().eq("id", id);
  revalidatePath("/admin/elimplay/categorias");
}

export default async function AdminElimPlayCategoriasPage() {
    ...
    <div>
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold" style={{ color: "var(--color-text)" }}>
          Categorías de ElimPlay
        </h1>
        <p className="mt-1 text-sm" style={{ color: "var(--color-text-muted)" }}>
          Organiza los audios por categoría: cantos, coros, temas, testimonios, etc.
        </p>
      </div>
