...
export default async function EditAudioTrackPage({ params }: Props) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {/* Cover preview */}
      <div
        className="mb-6 w-32 h-32 rounded-xl overflow-hidden flex items-center justify-center"
        style={{ background: "var(--color-surface-elevated)" }}
      >
        {a.cover_url ? (
          <img src={a.cover_url} alt={a.title} className="w-full h-full object-cover" />
        ) : (
          <Music size={32} style={{ color: "var(--color-text-muted)" }} />
        )}
      </div>

      {/* Audio preview */}
      <div className="mb-6">
        <audio controls src={a.audio_url} className="w-full" style={{ height: "40px" }} />
      </div>
      ...
      <form action={handleUpdate} className="flex flex-col gap-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="alabanza, coro juvenil, 2026 (separadas por coma)"
            className="w-full rounded-xl px-4 py-3 text-sm outline-none"
            style={inputStyle}
          />
        </div>
      ...
      >
        ...
        <p className="text-xs mb-4" style={{ color: "var(--color-text-muted)" }}>
          Eliminar el audio es irreversible. El archivo en Storage no se elimina automáticamente.
        </p>
