...
interface RuletaVoiceChatProps {
  codigo: string;
}
...
export function RuletaVoiceChat({ codigo }: RuletaVoiceChatProps) {
  const [state, setState] = useState<State>({ status: "idle" });

  async function handleJoinVoice() {
    setState({ status: "connecting" });
    try {
      const res = await fetch(`/api/ruleta/${codigo}/voice-token`, { method: "POST" });
