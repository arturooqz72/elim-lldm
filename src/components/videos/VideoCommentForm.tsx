...
export function VideoCommentForm({ videoId, userId }: { videoId: string; userId: string | null }) {
  ...
  if (!userId) {
    return (
      ...
      >
        ...
        <Link
          href={`/login?returnUrl=/videos/${videoId}`}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: insertError } = await supabase
      .from("video_comments")
      .insert({ video_id: videoId, user_id: userId, mensaje: texto });

    setSubmitting(false);

    if (insertError) {
      setError("No se pudo publicar tu comentario. Intenta de nuevo.");
      return;
    }
